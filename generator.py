...
[글 꾸미기 규칙 — 반드시 적용]
- 소제목(##): 각 섹션마다 사용. 궁금증을 유발하는 형태로. 앞에 이모지 1개 붙이기 (예: 🔍 왜 이런 일이 생길까요?)
- **굵게**: 핵심 키워드, 숫자, 중요한 메시지는 반드시 굵게 처리 (예: **연간 4,000만원**, **3년째 방치**)
- > 인용구: 건물주/담당자의 실제 말투나 핵심 메시지에 사용 (예: > "전화만 안 왔으면 좋겠어요")
- 목록(- ): 3개 이상 나열할 때 사용. 짧고 명확하게
- 구분선(---): 큰 주제가 전환될 때 1회 사용
- 빈 줄: 문단 사이에 반드시 빈 줄 넣기 (가독성)
- 절대 금지: 색깔 글씨, 이모지 남발(소제목에만 1개), HTML 태그"""
...
[글 꾸미기 규칙 — 반드시 적용]
- H1(#): 제목 1개만
- H2(##): 각 섹션마다 사용. 앞에 이모지 1개 붙이기
- H3(###): 세부 항목에 사용
- **굵게**: 핵심 키워드, 숫자, 중요한 메시지는 반드시 굵게 처리
- > 인용구: 핵심 메시지나 전문가 의견에 사용
- 목록(- ): 3개 이상 나열할 때 사용
- 구분선(---): 큰 주제 전환 시 1회 사용
- 빈 줄: 문단 사이에 반드시 빈 줄 넣기"""
